...
function top_news_post_col2($title,$cat_id,$categorie_slug,$tag,$orderby,$order,$thumb,$meta,$limit,$excerpt,$small_post_list,$el_class) {
    $unqID = uniqid();
    $meta_description = (($meta === 'true') ? 'yes' : 'no');    
    $list_thumb = (($thumb === 'true') ? 'yes' : 'no');
    ?>
    <div id="post-widget-<?php echo esc_attr($unqID);?>" class="posts-widget block <?php echo esc_attr($el_class); ?>">
        <?php if(! empty($title)) : ?>
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <?php
                if ($small_post_list == 'style-2') {
                    top_news_small_posts_list_v2(array(
                        'cat_ids'       => $cat_id,
                        'tag'           => $tag,
                        'limit'         => $limitrow,
                        'order'		=> $order,
